...
function getAccountBalance_(accountId) {
  ...
  if (txLastRow > 1) {
    // Mirrors the frontend's computeBalancesFromLists: an unconfirmed Income transaction
    // (auto-generated by a recurring item) hasn't landed yet, so it's excluded. Unconfirmed
    // Expense still counts, deliberately, as a safety net against overspending.
    txSheet.getRange(2, 1, txLastRow - 1, 9).getValues().forEach(function (row) {
      if (String(row[1]) !== String(accountId)) return;
      var confirmed = row[8] === true || String(row[8]).toUpperCase() === 'TRUE';
      if (row[3] === 'Income') {
        if (confirmed) incomeSum += Number(row[6]);
      } else if (row[3] === 'Expense') {
        expenseSum += Number(row[6]);
      }
    });
  }
